...
// Function to add content after the 'Unreleased' section in the CHANGELOG.md
...
async function readFragments() {
  ...
  for (const fragmentFilename of fragmentPaths) {
    ...
    if (
      !fragmentFilename.name.endsWith(".yml") &&
      !fragmentFilename.name.endsWith(".yaml")
    ) {
...
async function moveFragments(fragmentPaths: Dirent[]): Promise<void> {
  ...
  for (const fragmentFilename of fragmentPaths) {
    const fragmentPath = resolve(fragmentDirPath, fragmentFilename.name);
    const fragmentTempPath = resolve(
      fragmentTempDirPath,
...
function generateChangelog(sections: Changelog) {
  ...
  const changelogSections = Object.entries(sections).map(
    ([sectionKey, entries]) => {
